...
size_t get_placed_texture_storage_size(const rsx::texture &texture, size_t rowPitchAlignement, size_t mipmapBlockAlignment)
{
	...
	for (unsigned mipmap = 0; mipmap < texture.mipmap(); ++mipmap)
	{
		...
		result += align(rowPitch * heightInBlocks * d, mipmapBlockAlignment);
