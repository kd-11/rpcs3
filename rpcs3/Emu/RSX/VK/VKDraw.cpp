...
bool VKGSRender::bind_texture_env()
{
	...
	for (u32 textures_ref = current_fp_metadata.referenced_textures_mask, i = 0; textures_ref; textures_ref >>= 1, ++i)
	{
		...
		else
		{
			...
			const auto null_view = vk::null_image_view(*m_current_command_buffer, view_type);
			m_program->bind_uniform({ vk::null_sampler(), null_view->value, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, null_view->image()->value },
				vk::glsl::binding_set_index_fragment,
				m_fs_binding_table->ftex_location[i]);
...
bool VKGSRender::bind_interpreter_texture_env()
{
	...
	{
		vk::null_sampler(),
		vk::null_image_view(*m_current_command_buffer, VK_IMAGE_VIEW_TYPE_1D)->value,
		VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
		vk::null_image_view(*m_current_command_buffer, VK_IMAGE_VIEW_TYPE_1D)->image()->value
	};
